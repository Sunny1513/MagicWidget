...
import com.sun.datepickerview.model.LunarCalendarVo;
...
public class LunarCalendar {
     ...
     *
     * @param year        农历年份
     * @param month       农历月 实际get(Calendar.MONTH)时应该+1
     * @param monthDay    农历日
     ...
     * 农历转公历Calendar
     * @param calendar
     * @param isLeapMonth 是否闰月（同年两个月）
     * @return
     */
    public static Calendar getSolar(Calendar calendar, boolean isLeapMonth) {
        int month = calendar.get(Calendar.MONTH) + 1;
//        boolean isLeapMonth = leapMonth(calendar.get(Calendar.YEAR)) == month;
        int[] date = lunarToSolar(calendar.get(Calendar.YEAR), month,
                calendar.get(Calendar.DATE), isLeapMonth);
        Calendar calendar2 = Calendar.getInstance();
        calendar2.set(date[0], date[1] - 1, date[2], calendar.get(Calendar.HOUR_OF_DAY),
     ...
     * @return
     */
    public static LunarCalendarVo getLunar(Calendar calendar) {
        int[] date = solarToLunar(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH) + 1, calendar.get(Calendar.DATE));
        Calendar calendar2 = Calendar.getInstance();
        calendar2.set(date[0], date[1] - 1, date[2], calendar.get(Calendar.HOUR_OF_DAY),
                calendar.get(Calendar.MINUTE), calendar.get(Calendar.SECOND));
        return new LunarCalendarVo(calendar2, date[3] == 1);
    }
     ...
     * @param month
     * @param monthDay
     * @return 返回公历日期对应的农历日期，year0，month1，day2，leap3(1true)
     */
    ...
    public static int leapMonth(int year) {
        ...
        if (index >= LUNAR_INFO.length) {
            return 0;
        }
        return (LUNAR_INFO[index] & 0xF00000) >> 20;
    }
